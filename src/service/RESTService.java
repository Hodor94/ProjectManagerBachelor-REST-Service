...
public class RESTService {
    ...
    private final String TOKEN_CREATION_ERROR = "TOKEN CREATION FAILED!";
    ...
    public JSONObject loginUser(JSONObject userInformation) {
        ...
        try {
            ...
            if (dataService.login(username, password)) {
                ...
                if (token != null && !(token.equals(TOKEN_CREATION_ERROR))) {
                    jsonInfo = "{\"success\": \"true\", \"token\": \"" + token +
                            "\", " + "\"user\": " + user.toSring() + "}";
                    result = new JSONObject(jsonInfo);
                } else {
                    jsonInfo = "{\"success\": \"false\", \"reason\": \"" + TOKEN_CREATION_ERROR + "\"}";
                }
            } else {
                jsonInfo = "{\"success\": \"false\", \"reason\": \"Login information is wrong!\"}";
                result = new JSONObject(jsonInfo);
            }
        } catch (JSONException e) {
            result = returnClientError();
    ...
    private String createUserToken(String userid, String username,
        ...
        String result = null;
        // If not called the first time, keys are set
        if (privateKey != null && publicKey != null) {
            long currentMilliseconds = System.currentTimeMillis();
            Date creationTime = new Date(currentMilliseconds);
            Date expireTime = new Date(currentMilliseconds + 900000); // 15 minutes after creating the token the
            // token expires

            // Create header with information about key algorithm
            JWSHeader header = new JWSHeader(JWSAlgorithm.RS256);

            // Create body of token with information
            JWTClaimsSet claimsSet = new JWTClaimsSet.Builder()
                    .subject("authentication")  // Defines for what the tokens are used
                    .audience("users")  // Defines the audience which uses the token
                    .issueTime(creationTime)    // Saves the creation time of the token
                    .expirationTime(expireTime) // Saves the time when the token expires
                    .jwtID(userid)  // Saves the userid of the logged in user
                    .claim("username", username)    // Saves the username
                    .claim("role", userRole)    // Saves the role of the user
                    .claim("team", teamName)    // Saves the name of the team in which the user takes part
                    .build();

            // Put together header and body in an encrypted jwt
            SignedJWT token = new SignedJWT(header, claimsSet);

            // Create a signer for signature of the token. Use private key for signature.
            ...
            try {
                ...
                e.printStackTrace();
                result = TOKEN_CREATION_ERROR;
            }
        } else {
            // Keys are not set. Generate keys and call the method again
            generateKeys();
            createUserToken(userid, username, userRole, teamName);
        }
        return result;
    }
    ...
    private void generateKeys() {
        try {
            ...
            keyGenerator.initialize(2048); // Research https://crypto.stackexchange.com/questions/1978/how-big-an-rsa-key-is-considered-secure-today
    ...
    public String validateToken(String token) {
        if (publicKey != null) {
            try {
                ...
                e.printStackTrace();
                return "TOKEN COULD NOT BE PARSED!";
            } catch (JOSEException e) {
                e.printStackTrace();
